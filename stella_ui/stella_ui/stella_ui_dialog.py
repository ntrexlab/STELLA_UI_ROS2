...
from stella_ui_msg.msg import RobotStatus
...
class MyFrame(rtmgr.MyFrame):
	...
	def __init__(self, *args, **kwds):
		...
		self.t_sub = threading.Thread(target = self.subscription)
		...
		#
		rclpy.init()
		self.node = rclpy.create_node('stella_ui')
#		self.node.create_subscription(std_msgs.msg.String, 'to_rtmgr', self.ROSCb, 10)
#		self.pub = self.node.create_publisher(std_msgs.msg.String , 'from_rtmgr' , 10 )
		self.t_robot.start()
		self.sub_pose = self.node.create_subscription(PoseWithCovarianceStamped, 'amcl_pose', self.robot_status_data_position,10)
		self.sub_goal = self.node.create_subscription(PoseStamped, 'move_base_simple/goal', self.robot_status_data_goal,10)
		self.sub_vel = self.node.create_subscription(Twist, 'cmd_vel', self.robot_status_data_vel,10)
		self.t_sub.start()
...
#		r.sleep()
	...
	def OnConnect(self, event):
		...
		if v == True:
			self.robot_connect_proc = psutil.Popen(['sshpass', '-p', robot_password, 'ssh', robot_id + '@' + robot_ip, '-oStrictHostKeyChecking=no', 'source /opt/ros/foxy/setup.bash && source ~/colcon_ws/install/local_setup.bash && ros2 launch stella_bringup robot.launch.py'], stdout=out, stderr=err)
		elif v == False:
			self.robot_connect_proc = psutil.Popen(['sshpass', '-p', robot_password, 'ssh', robot_id + '@' + robot_ip, '-oStrictHostKeyChecking=no', 'source /opt/ros/foxy/setup.bash && source ~/colcon_ws/install/local_setup.bash && killall -9 stella_md_node stella_ahrs_node ydlidar_node joint_state_pub robot_state_pub'], stdout=out, stderr=err)
	...
	def OnSaveMap(self, event):
		...
		if v == True:
			self.robot_slam_proc = psutil.Popen("ros2 run nav2_map_server map_saver_cli -f " + self.path_save_map + "/map",stdin=subprocess.PIPE, stdout=out, stderr=err, shell=True)
		elif v == False:
	...
	def robot_status(self):
		while rclpy.ok():
			pub = self.node.create_publisher(RobotStatus,'robot_status',1)
...
#				print 'Err scheduling_policy()'
	...
	def publish_param_topic(self, pdic, prm):
		...
		if 'stamp' in prm.get('flags', []):
			...
			setattr(obj, attr, self.node.get_clock().now())
...
#				time.sleep(0.05)
	...
	def topics_proc_th_start(self, topic):
		out = subprocess.PIPE
		err = subprocess.STDOUT
		self.topics_echo_proc = psutil.Popen([ 'ros2','topic', 'echo', topic ], stdout=out, stderr=err)
	...
	def status_th(self, ev):
		while self.button_message_on.GetValue() == True:
				self.robot_cur_x_msg = self.robot_cur_x
				self.robot_cur_y_msg = self.robot_cur_y
				self.robot_goal_x_msg = self.robot_goal_x
				self.robot_goal_y_msg = self.robot_goal_y
				self.robot_linear_speed_msg = self.robot_linear_speed
				self.robot_angular_speed_msg = self.robot_angular_speed
				time.sleep(0.1)
...
#                pub = rospy.Publisher('state_cmd', std_msgs.msg.Int32, queue_size=10)
		pub = self.node.create_publisher(std_msgs.msg.Int32, 'state_cmd', 10)
...
#		ros_topics = rospy.get_published_topics()
		ros_topics = self.node.get_topic_names_and_types()
